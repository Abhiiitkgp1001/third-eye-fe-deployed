...
import { SIGNAL_GROUPS } from "@/lib/movements";
...
export default function SignalsList({ movementDefinitions }: SignalsListProps) {
  ...
  // Get readable label for each signal
  const allKnownSignals = SIGNAL_GROUPS.flatMap((g) => g.signals);

  const signalsWithMetadata = movementDefinitions.map((def) => {
    const knownSignal = allKnownSignals.find((s) => s.key === def.name);

    return {
      name: def.name,
      label: knownSignal
        ? knownSignal.label
        : def.name.split('_').map(word => word.charAt(0) + word.slice(1).toLowerCase()).join(' '),
      description: def.description,
      isCustom: !knownSignal,
    };
  });
